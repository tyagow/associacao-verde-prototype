"""Phase 3 screenshot capture for /equipe (Team Command).

Captures the rebuilt command center (TeamShell + KPIs + activity feed +
Pix-by-hour + priority queue) at:
  - desktop 1280x800
  - mobile  390x844

Outputs:
  artifacts/visual-e2e/redesign/p3-team-command-desktop.png
  artifacts/visual-e2e/redesign/p3-team-command-mobile.png

Usage:
  python3 scripts/p3-screenshots.py
  E2E_BASE_URL=http://127.0.0.1:4184 python3 scripts/p3-screenshots.py
"""
...
def start_isolated_server() -> tuple[str, subprocess.Popen, tempfile.TemporaryDirectory]:
    ...
    tmp = tempfile.TemporaryDirectory(prefix="p3-screenshots-")
    env = {
        **os.environ,
        "PORT": str(port),
        "NEXT_DEV": "false",
        "DB_FILE": str(Path(tmp.name) / "p3.sqlite"),
...
def stage_activity(base_url: str, page: Page) -> None:
    """Trigger a patient login + checkout so the activity feed and Pix-by-hour
    chart have at least one event to render."""
    page.goto(f"{base_url}/paciente", wait_until="domcontentloaded")
    try:
...
def shoot(page: Page, name: str, viewport: dict) -> Path:
    ...
    target = OUT / f"p3-team-command-{name}.png"
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                try:
                    stage_activity(base, stage_page)
                except Exception as exc:
                    print(f"[warn] could not stage activity: {exc}")
                stage_ctx.close()
