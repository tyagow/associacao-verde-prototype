"""Phase 6 screenshot capture for /equipe/estoque.

Captures the rebuilt single product ledger + Cultivo panel at:
  - desktop 1280x800
  - mobile  390x844

Outputs:
  artifacts/visual-e2e/redesign/p6-stock-desktop.png
  artifacts/visual-e2e/redesign/p6-stock-mobile.png
...
pattern as scripts/p4-screenshots.py).

Usage:
  python3 scripts/p6-screenshots.py
  E2E_BASE_URL=http://127.0.0.1:4184 python3 scripts/p6-screenshots.py
"""
...
def start_isolated_server() -> tuple[str, subprocess.Popen, tempfile.TemporaryDirectory]:
    ...
    tmp = tempfile.TemporaryDirectory(prefix="p6-screenshots-")
    env = {
        **os.environ,
        "PORT": str(port),
        "NEXT_DEV": "false",
        "DB_FILE": str(Path(tmp.name) / "p6.sqlite"),
...
def shoot(page: Page, name: str, viewport: dict) -> Path:
    ...
    target = OUT / f"p6-stock-{name}.png"
...
def main() -> None:
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                # Desktop screenshot (also expand the first product so the
                # lot detail + inline-edit grid is visible in the capture).
                ctx = browser.new_context(viewport=DESKTOP)
                page = ctx.new_page()
                login_team(page, base)
                page.goto(f"{base}/equipe/estoque", wait_until="networkidle")
                page.wait_for_timeout(800)
                try:
                    page.locator("[data-product-id]").first.click(timeout=4000)
                    page.wait_for_timeout(400)
                except Exception as exc:
                    print(f"[warn] could not expand first product: {exc}")
